...
struct EditDraftView: View {
    ...
    private func loadDraft() {
        ...
        nodes = draft.nodes.map { draftNode in
            StyledNode(
                id: draftNode.id,
                text: draftNode.text,
        ...
        connections = draft.connections.map { draftConn in
            StyledConnection(
                id: draftConn.id,
                fromNodeId: draftConn.fromNodeId,
                toNodeId: draftConn.toNodeId,
                reason: draftConn.reason ?? "",
                style: .defaultStyle
            )
        }
    ...
    private func updateDraft() {
        ...
        DraftManager.shared.updateDraft(updatedDraft)
...
// MARK: - 下書き子ノード表示View
struct DraftTreeChildNodesView: View {
    let parentId: UUID
    @Binding var nodes: [StyledNode]
    @Binding var connections: [StyledConnection]
...
// MARK: - 下書き接続理由入力
struct DraftConnectionReasonInput: View {
    @Binding var connection: StyledConnection
    let scale: CGFloat
    let level: Int
...
// MARK: - 下書きビジュアルモードView
struct DraftVisualModeView: View {
...
// MARK: - 下書きスタイル付きノードView
struct DraftStyledNodeView: View {
    @Binding var node: StyledNode
    let centerNodeText: String
    let isSelected: Bool
    let canDrag: Bool
...
// MARK: - 下書きスタイル付き接続線
struct DraftStyledConnectionLine: View {
    @Binding var connection: StyledConnection
